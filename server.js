...
function manualSuggestions(message) {
  ...
  if (m.includes("باب") || m.includes("door") || m.includes("doors")) {
    ...
    s.push({ label: "👥 جروب الأبواب", send: "جروب الأبواب" }); // ✅ التعديل المطلوب
    return s;
  }
...
app.post("/chat", (req, res) => {
  try {
    ...
    if (isGreeting(message)) {
      return res.json({
        reply: (K.greetings?.reply || "أهلاً 👋"),
        context: nextContext,
        suggestions: [
          { label: "📄 دليل استخدام المنتجات", send: "دليل استخدام المنتجات" }, // ✅ التعديل المطلوب
          { label: "📄 دليل كارت", send: "دليل كارت" },
          { label: "📄 دليل باب", send: "دليل باب" },
          { label: "📄 دليل كامة", send: "دليل كامة" },
          { label: "☎️ أرقام التواصل", send: "أرقام التواصل" },
          { label: "☎️ الخط الساخن", send: "الخط الساخن" }, // ✅ إضافة
          { label: "👤 خدمة العملاء", send: "خدمة العملاء" }, // ✅ إضافة
          { label: "🧩 جروبات دعم كاس", send: "جروبات دعم كاس" }, // ✅ إضافة
          { label: "⚠️ رموز الأعطال", send: "رموز الأعطال" }, // ✅ إضافة
    ...
    // ✅ الخط الساخن
    if (m === "الخط الساخن" || m.includes("الخط الساخن") || m.includes("الخط الساخن")) {
      return res.json({
        reply: `☎️ الخط الساخن:\n${K.hotline || "غير متاح حالياً."}`,
        context: nextContext
      });
    }

    // ✅ خدمة العملاء
    if (m === "خدمة العملاء" || m.includes("خدمه العملاء")) {
      const deps = safeObj(K.departments);
      const cs = deps["خدمة العملاء"];
      const admin = safeArray(cs?.admin).filter(Boolean);
      const num = admin[0] || "غير متاح حالياً.";
      return res.json({
        reply: `👤 خدمة العملاء:\n${num}`,
        context: nextContext
      });
    ...
    // ✅ رموز الأعطال
    if (m.includes("رموز الاعطال") || m.includes("رموز الأعطال")) {
      return res.json({
        reply: `⚠️ رموز الأعطال:\n${K.malfunctionsUrl || ""}`,
        context: nextContext
      });
    }

    // ✅ جروبات دعم كاس (قائمة)
    if (m === "جروبات دعم كاس" || m.includes("جروبات دعم كاس")) {
      return res.json({
        reply: "اختار الجروب اللي محتاجه 👇",
        context: nextContext,
        suggestions: [
          { label: "👥 جروب الأبواب", send: "جروب الأبواب" },
          { label: "🛠️ جروب الدعم الفني", send: "جروب الدعم الفني" }
        ]
      });
    }
    ...
    if (m.includes("جروب") && (m.includes("باب") || m.includes("ابواب") || m.includes("الأبواب"))) {
      return res.json({
        reply: `جروب كاس للدعم الفني للأبواب:\n${K.autoDoorSupportGroup?.url || ""}`,
        context: nextContext
      });
    }

    // ✅ جروب الدعم الفني
    if (m.includes("جروب") && (m.includes("الدعم الفني") || (m.includes("دعم") && m.includes("فني")))) {
      return res.json({
        reply: `جروب كاس للدعم الفني:\n${K.techSupportGroup?.url || ""}`,
        context: nextContext
      });
    }

    /* ✅ تعديل بسيط: "أرقام التواصل" بدل "الأرقام" */
    if (
      m === "ارقام التواصل" || m === "أرقام التواصل" ||
      m === "ارقام تواصل" || m === "أرقام تواصل"
    ) {
      return res.json({
        reply: "اختار القسم علشان تظهر لك كل أرقام التواصل 👇",
        context: nextContext,
    ...
    if (isManualIntent(message)) {
      ...
      const isCategoryOnly =
        ...
        m === "أدلة" ||
        m === "أدلة الاستخدام" ||
        m === "دليل استخدام المنتجات" || // ✅ إضافة
        m === "دليل المنتجات" || // ✅ إضافة
        m === "دليل كارت" ||
        m === "دليل باب" ||
        m === "دليل كامه" ||
        m === "دليل كامة";
    ...
    // fallback
    return res.json({
      reply: "اكتب: (دليل استخدام المنتجات) أو (دليل كارت/باب/كامة) أو اسم المنتج.",
      context: nextContext,
      suggestions: [
        { label: "📄 دليل استخدام المنتجات", send: "دليل استخدام المنتجات" },
        { label: "📄 دليل كارت", send: "دليل كارت" },
        { label: "📄 دليل باب", send: "دليل باب" },
        { label: "📄 دليل كامة", send: "دليل كامة" },
        { label: "☎️ أرقام التواصل", send: "أرقام التواصل" },
        { label: "☎️ الخط الساخن", send: "الخط الساخن" },
        { label: "👤 خدمة العملاء", send: "خدمة العملاء" },
        { label: "🧩 جروبات دعم كاس", send: "جروبات دعم كاس" },
        { label: "⚠️ رموز الأعطال", send: "رموز الأعطال" },
        { label: "🛒 المتجر", send: "المتجر" },
        { label: "📍 عناوين الفروع", send: "عناوين الفروع" }
      ]
    });
